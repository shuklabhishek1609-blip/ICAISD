...
\usepackage{xcolor}
\usepackage{graphicx}
% ICAISD checklist: references are cited in text as a SUPERSCRIPT [n].
% acmart's default numeric style sets them inline, so override natbib.
...
\email{jonathanbokeunchang@gmail.com}
\affiliation{%
  \institution{Union County Magnet High School}
  \city{Scotch Plains}
  \state{New Jersey}
...
(Section~\ref{sec:analytic}). Whether accuracy falls in proportion is precisely
...
(Figure~\ref{fig:pareto-train}): error falls as energy rises, from Historical
Last at zero energy through LSTM and STGCN to Graph WaveNet. No model is strictly dominated on that axis, and
...
Measuring the same four models on inference energy rather than training energy
reproduces that shape and that ordering exactly (Figure~\ref{fig:pareto-infer}),
which is the agreement between measured axes that Section~\ref{sec:proxy} turns
...
Figure~\ref{fig:horizon} plots the four curves. This produces a crossover
worth noting. At the first horizon step STGCN's
...
\begin{acks}
Use of AI: the authors used a large language model as a coding and drafting
...
by the authors; every reported measurement was produced by executing the
